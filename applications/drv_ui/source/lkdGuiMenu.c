...
/**
  ...
  *@retval y坐标
  */
static uint16_t GetCheckedNodeDrawy(lkdMenu *pM, uint8_t index, uint16_t cy)
{
	uint8_t cNum;
	cNum = (pM->hight - 1 - (cy - pM->y)) / pM->Itemshigh;
	if(index >= cNum){
		return (uint16_t)(cy + (cNum - 1)*pM->Itemshigh);
	}
	else{
		return (uint16_t)(cy + index*pM->Itemshigh);
	}
}
...
/**
  ...
  *@retval 绘制节点数
  */
static uint8_t GetDrawNodes(const lkdMenu *pM,const lkdMenuNode *pDraw,uint16_t *drawy)
{
...
static uint8_t GetDrawNodesNum(const lkdMenu *pM,const lkdMenuNode *pDraw,uint16_t drawy)
{
	uint8_t nodeNum,DrawNodeNum;
	nodeNum = GetbNodesNum(pDraw);
	DrawNodeNum = (pM->hight - 1 - (drawy - pM->y)) / pM->Itemshigh;
...
/**
  ...
  *@retval None
  */
static void DrawNodes(lkdMenu *pM,lkdMenuNode *pthisNode,uint16_t x,\
	uint16_t beginy,uint8_t index,uint8_t cNum)
{
