...
/**
  ...
  *@retval None
  */
void GuiBiasLine(uint16_t x0, uint16_t y0,\
	uint16_t x1, uint16_t y1, uint16_t color)
{
	...
	if(x0 == x1){
		...
		if((y1 - y0) < (x1 - x0)){
			...
			while(1){
				GuiDrawPoint(x0, (uint16_t)tf, color);
				x0 ++;
				tf += offset;
				if(tf > y1){
					tf = y1;
			...
			while(1){
				GuiDrawPoint((uint16_t)tf, y0, color);
				y0 ++;
				tf += offset;
				if(x0 > x1){
					x0 = x1;
...
/**
  ...
  *@retval None
  */
void GuiHPointLine(uint16_t x0, uint16_t y0,\
	uint16_t x1, uint8_t interval, uint16_t color)
{
	uint16_t tx;
...
/**
  ...
  *@retval None
  */
void  GuiRPointLine(uint16_t x0, uint16_t y0,\
	uint16_t y1,uint8_t interval, uint16_t color)
{
	uint16_t ty;
...
/**
  *@brief 画二维码位图
  *@param  bitMap 位图数组，紧凑排列
  *@param  x0,y0 起始坐标
  *@param  len 二维码宽度和高度相同
  *@param  flag 0 正常显示 1反显
  *@retval None
  */
void GuiQRCodeBitMap(uint8_t *bitMap,uint16_t x0,
	uint16_t y0,uint32_t len, uint8_t flag)
{
    ...
    for (y = 0; y < len; y++)
    {
        for (x = 0; x < len; x++)
        {
            uint32_t offset = y * len + x;
            colour = (bitMap[offset >> 3] & (1 << (7 - (offset & 0x07)))) != 0;
            GuiDrawPoint(x0 + x,y0 + y, colour);
        }
    }
}
